...
Nhờ kiến trúc **Ports & Adapters** của CycleWarden, tất cả các gói logic nghiệp vụ của hệ thống (`@cyclewarden/db`, `@cyclewarden/auth`, `@cyclewarden/security`, `@cyclewarden/mail`, `@cyclewarden/storage`, `@cyclewarden/logger`, `@cyclewarden/i18n`, `@cyclewarden/payment`) đều hoàn toàn **framework-agnostic** (độc lập với framework UI).
...
Dưới đây là các bước để tích hợp ứng dụng Nuxt.js (`apps/nuxt`) vào CycleWarden monorepo:
...
```json
{
  ...
  "dependencies": {
    "@cyclewarden/db": "workspace:*",
    "@cyclewarden/auth": "workspace:*",
    "@cyclewarden/security": "workspace:*",
    "@cyclewarden/storage": "workspace:*",
    "@cyclewarden/mail": "workspace:*",
    "@cyclewarden/logger": "workspace:*",
    "@cyclewarden/i18n": "workspace:*",
    "nuxt": "^3.11.0"
  }
}
```
...
2. **CORS**: Nếu hai ứng dụng gọi API chéo của nhau, hãy cấu hình whitelist CORS của các endpoints bằng cách sử dụng `@cyclewarden/security` để đảm bảo an toàn.
