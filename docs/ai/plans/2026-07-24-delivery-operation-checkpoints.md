...
Each protected command writes an integrity-covered `operation.json` containing:
...
An atomically-created `operation.lock` directory prevents a second CLI operation from starting for the same cycle.

## Recovery rule

Inspection is read-only. Explicit `--apply` may clear a lease only when the checkpoint integrity passes, it belongs to the current host, and both recorded PIDs are dead.

A live owner, live child, foreign-host lease, invalid checkpoint, or orphan lock without a checkpoint remains blocked.
